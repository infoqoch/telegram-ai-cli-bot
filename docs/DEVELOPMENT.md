...
- [`install_power_manager.sh`](../scripts/macos/install_power_manager.sh) renders the committed plist templates into the current user's `~/Library/LaunchAgents`, then registers them with `launchctl bootstrap`. Uninstall uses `bootout` and does not alter the current bot process state.
- The installer snapshots `power_manager.sh` under `.data/power-management/runtime/` after stopping the old LaunchAgents. Installed jobs execute this immutable runtime copy, preventing an in-place project update from changing a shell script while its monitor process is still reading it.
...
- A separate `com.telegram-ai-cli-bot.daily-start` calendar LaunchAgent invokes `power_manager.sh daily-start` at 03:00. The command atomically records `desired=on`, waits up to five minutes for any active reconciliation, and then reuses the normal reconciliation path. Keeping this one-shot job separate is required because the monitor job is normally already running.
- Event-stream reconciliations run as separate `power_manager.sh reconcile` processes so lock ownership identifies the short-lived reconciliation process rather than the long-lived monitor shell.
