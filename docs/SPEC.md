...
| `Codex` | ChatGPT login-based CLI coding agent | `sol-xhigh`, `sol-high`, `astra-xhigh` |
...
| Claude | `Haiku` | `haiku` |
| Codex | `Sol XHigh` | profile key `sol-xhigh`, current backing `model=gpt-5.6-sol`, `reasoning=xhigh` |
| Codex | `Sol High` | profile key `sol-high`, current backing `model=gpt-5.6-sol`, `reasoning=high` |
| Codex | `Astra XHigh` | profile key `astra-xhigh`, current backing `model=gpt-6-astra`, `reasoning=xhigh` |
...
- Codex stores provider-local profile keys (`sol-xhigh`, `sol-high`, `astra-xhigh`) like Claude stores `opus` / `sonnet` / `haiku`; concrete GPT versions are selected inside `src/ai/catalog.py`.
- Legacy Codex keys such as `xhigh`, `high`, `medium`, `terra-xhigh`, `codex_high`, `gpt54_high`, and `gpt54_xhigh` are accepted as aliases and normalized to the current provider-local keys.
...
**Inspect schedule history:** `/scheduler` → `History` shows a unified read-only schedule history and health dashboard. It lists total/active/attention counts, each visible schedule's next run, run count, last run, latest execution outcome, recent delivery status, and a normalized issue label such as target chat unavailable, Telegram timeout/network/circuit failure, provider authentication failure, provider command failure, command timeout/failure, delivery abandoned after retries, or worker stopped. The history view sorts ON schedules first, then by latest last run. Numbered detail buttons match the numbered message order.

Schedule executions are recorded in `schedule_runs`, separate from `message_log`. `schedule_runs` stores execution outcomes such as `success`, `no_output`, `warning`, `delivery_failed`, or `failed`; `message_log` remains the delivery/response record only when a message exists.

Plugin scheduled actions may return metadata-only dicts such as `{"text": null, "run_status": "warning", "summary": "...", "error": "..."}`. The bot skips Telegram delivery, does not create a `message_log`, and records the warning in `schedule_runs`.
